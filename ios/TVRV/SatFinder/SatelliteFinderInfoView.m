...
{
    if (IS_IPAD)
        self = [super initWithFrame:CGRectMake(423.0, 0.0, 345.0, 514.0)];
    else
        self = [super initWithFrame:CGRectMake(0.0, 0.0, ApplicationDelegate.screenSize.width, ApplicationDelegate.screenSize.height-54)];
    ...
    if (self)
    {
        ...
        UITextView* textView;
        if (IS_IPAD)
            textView = [[UITextView alloc] initWithFrame:CGRectMake(20.0, 85.0, 281.0, 350.0)];
        else
            textView = [[UITextView alloc] initWithFrame:CGRectMake((ApplicationDelegate.screenSize.width-303)+4, 65.0, 281.0, 280.0)];
            //textView = [[UITextView alloc] initWithFrame:CGRectMake(20.0, 65.0, 281.0, 280.0)];
        ...
        if (IS_IPAD)
            [okButton setFrame:CGRectMake(90.0, 456.0, 135.0, 46.0)];
        else
            okButton.center = CGPointMake(textView.center.x, 360.0+(okButton.frame.size.height/2));

        //[okButton setFrame:CGRectMake((textView.center, 360.0, 128.0, 43.0)];
           // [okButton setFrame:CGRectMake(96.0, 360.0, 128.0, 43.0)];
...
{
  if (IS_IPAD)
        [[UIImage imageNamed:@"sf_info_bg.png"] drawInRect:CGRectMake(0.0, 58.0, 322.0, 456.0)];
	else
        [[UIImage imageNamed:@"sf_info_bg.png"] drawInRect:CGRectMake((ApplicationDelegate.screenSize.width-303)-9, 42.0, 303.0, 374.0)];
}
