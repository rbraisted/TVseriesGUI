...
function RH_ShowHelp(hParent, a_pszHelpFile, uCommand, dwData)
{
	console.log('RH_ShowHelp('+hParent+', '+a_pszHelpFile+', '+uCommand+', '+dwData+')');
	// this function only support WebHelp
	var strHelpPath = a_pszHelpFile;
	var strWnd = "";
	var nPos = a_pszHelpFile.indexOf(">");
...
{
	console.log('RH_ShowWebHelp('+hParent+', '+strHelpPath+', '+strWnd+', '+uCommand+', '+dwData+')');
	// hParent never used.
...
{
	console.log('ShowWebHelp('+strHelpPath+', '+strWnd+', '+uCommand+', '+nMapId+')');
	var a_pszHelpFile = "";
	if (uCommand == HH_DISPLAY_TOPIC)
	{
		a_pszHelpFile = strHelpPath + "#<id=0";
	...
	if (a_pszHelpFile)
	{
		...
		{
			console.log('here\'s where we do the window.open('+a_pszHelpFile+', "__webCshStub", '+sParam+')');
			var sParam = "left="+screen.width+",top="+screen.height+",width=100,height=100";
			window.open(a_pszHelpFile, "__webCshStub", sParam);
		}
	}
...
{
	console.log('loadData('+sFileName+')');
	if(!getElement("dataDiv"))
	{
		if(!insertDataDiv())
		{
